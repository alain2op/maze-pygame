...
def tile_checker(maze,tile):
    check=True
    row,col,floor=int(tile[0]),int(tile[1]),int(tile[2])
    neighbors=[]
    rows, cols,floors = maze.shape
    if col < cols - 1:
        neighbors.append(maze[row, col + 1,floor])
    if col > 0:
        neighbors.append(maze[row, col - 1,floor])
    if row > 0:
        neighbors.append(maze[row - 1, col,floor])
    if row < rows - 1:
        neighbors.append(maze[row + 1, col,floor])
    if floor < floors - 1:
        neighbors.append(maze[row, col,floor+1])
    if floor > 0:
        neighbors.append(maze[row, col,floor-1])
...
def path_generator(size,start,end):
    ...
    while(tile[0]!=end[0] or tile[1]!=end[1]):
        ...
        if(x_edge and y_edge and z_edge):
            ...
            maze=np.ones((size,size,size))
            probs=probs_init[:]
            tile=np.array([(size-1)/2,(size-1)/2,(size-1)/2])
            maze[int(tile[0]),int(tile[1]),int(tile[2])]=0
            solution=open("path.txt","w")
        else:
...
def maze_generator(size):
    ...
    start=np.array([(size-1)/2,(size-1)/2,(size-1)/2])
    ...
    if vertex==0:
        ...
        end=np.array([size-1,0,0])
    elif vertex==2:
        end=np.array([size-1,size-1,0])
    elif vertex==3:
        end=np.array([0,size-1,0])
    elif vertex==4:
        end=np.array([0,0,size-1])
    elif vertex==5:
        end=np.array([size-1,0,size-1])
    elif vertex==6:
    ...
    for i in range(ITERATIONS):
        ...
        if((tile[0]/(size-1)==0 or tile[0]/(size-1)==1) and (tile[1]/(size-1)==0 or tile[1]/(size-1)==1) and (tile[2]/(size-1)==0 or tile[2]/(size-1)==1)):
            continue
        if(tile[0]/(size-1)==0):
            poss_moves.remove(3)
        if(tile[0]/(size-1)==1):
    ...
    complementary_endx,complementary_endy,complementary_endz=int(not(endx/(size-1)))*(size-1),int(not(endy/(size-1)))*(size-1),int(not(endz/(size-1)))*(size-1)
